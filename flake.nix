    ...
    in
    {
      apps."aarch64-darwin".default = mkInit {
        ...
        script = ''
          ...
          sudo -u "$SUDO_USER" nix --experimental-features 'nix-command flakes' run nix-darwin/nix-darwin-25.05#darwin-rebuild -- switch --flake "$DOTFILES_DIR#$FLAKE"
          ...
          sudo -u "$SUDO_USER" nix --experimental-features 'nix-command flakes' run home-manager/master -- switch --flake "$DOTFILES_DIR#$FLAKE"
